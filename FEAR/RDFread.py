...
class RDFread(object):
    ...
    def __postProc(self, atomlist, matrix):  #обработчик сырых данных. ставит НЭП на диагональ
        table = dict(H=1, D=1, He=2, Li=1, Be=2, B=3, C=4, N=5, O=6, F=7, Ne=8, Na=1, Mg=2, Al=3, Si=4, P=5, S=6,
                     Cl=7, Ar=8, K=1, Ca=2, Sc=3, Ti=4, V=5, Cr=6, Mn=7, Fe=8, Co=2, Ni=2, Cu=2, Zn=2,
                     Ga=3, Ge=4, As=5, Se=6, Br=7, Kr=8, Rb=1, Sr=2, Y=39, Zr=40, Nb=41, Mo=42, Tc=43, Ru=44,
                     Rh=45, Pd=46, Ag=1, Cd=2, In=3, Sn=4, Sb=5, Te=6, I=7, Xe=8, Cs=1, Ba=2, La=3, Ce=58,
                     Pr=59, Nd=60, Pm=61, Sm=62, Eu=3, Gd=3, Tb=3, Dy=3, Ho=3, Er=3, Tm=3, Yb=70, Lu=71, Hf=72,
                     Ta=73, W=74, Re=75, Os=76, Ir=77, Pt=78, Au=1, Hg=2, Tl=81, Pb=82, Bi=83, Po=84, At=85, Rn=86,
                     Fr=1, Ra=2, Ac=3, Th=3, Pa=3, U=3, Np=3, Pu=3, Am=3, Cm=3, Bk=3)
        ...
        for i, j in enumerate(atomlist):  #найдем число несвязанных электронов для каждого атома.
    ...
    def readdata(self):
        ...
        '''
        sp.call(['standardize', self.__RDFfile, '-c', os.path.join(os.path.dirname(__file__), 'standardizer.xml'), '-f', 'rdf', '-o', 'stand_tmp.rdf'])
        with open('stand_tmp.rdf') as f:
            ir = -1
            im = -1
            atomcount = -1
            bondcount = -1
            ...
            for n, line in enumerate(f):
                if not failkey and "$RXN" not in line[0:4]:
                    ...
                    if line[31:34].strip() == "H" and not int(line[60:63]) and not int(line[34:36]) and not int(line[38:39]):
                        pass
                    else:
